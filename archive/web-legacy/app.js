...
// Toast Notification Engine (Rule 05 & Luxury Aesthetic Compliant)
function showToast(message, type = 'lime', duration = 3500) {
    ...
    // Remove existing toast with identical message to prevent spam stacking
    Array.from(container.children).forEach(child => {
        const textSpan = child.querySelector('.toast-message');
        if (textSpan && textSpan.innerText.trim() === message.trim()) {
            child.remove();
        }
    });
    ...
    let iconSvg = ICONS.drop;

    if (message.toLowerCase().includes('cart') || message.toLowerCase().includes('added')) {
        badgeText = 'CART DROP';
        iconSvg = ICONS.cart;
    } else if (message.toLowerCase().includes('upload') || message.toLowerCase().includes('cloudinary')) {
        badgeText = 'CLOUD DROP';
        iconSvg = ICONS.cloud;
    } else if (message.toLowerCase().includes('collection') || message.toLowerCase().includes('drop')) {
        badgeText = 'EXCLUSIVE';
        iconSvg = ICONS.drop;
    }
    ...
    toast.innerHTML = `
        ...
        <button class="toast-close-btn" onclick="removeToastElement(this.parentElement)">${ICONS.close}</button>
    `;
